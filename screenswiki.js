...
function init(module, app, next) {
    calipso.lib.step(
       function defineRoutes(err) {
        ...
        // Default routes
        module.router.addRoute('GET /screens', showScreenContentList, {template: 'screenslist', block: 'screenslist', permit: vPerm}, this.parallel()); 
        module.router.addRoute('GET /screens/list', showScreenContentList, {template: 'screenslist', block: 'screenslist', permit: vPerm}, this.parallel()); 
        module.router.addRoute('GET /screens/view/id/:id', showScreenContentExpanded, {template: 'screenshow', block: 'screencontent32', permit: vPerm}, this.parallel()); 
        module.router.addRoute('GET /screens/view/alias/:alias.:html', showScreenContentExpanded, {template: 'screenshow', block: 'ignored', permit: vPerm}, this.parallel()); 
        module.router.addRoute('GET /screens/fullview/alias/:alias.:html', showScreenContentExpanded, {template: 'screenshow', block: 'ignored', permit: vPerm}, this.parallel()); 
	// module.router.addRoute('GET /screens/edit/alias/:alias.:html', editScreenContentFromAlias, {template: 'screenedit', block: 'screenedit', permit: vPerm}, this.parallel()); 
	//
        module.router.addRoute('GET /screens/edit/id/:id', editContentForm,{admin:true,permit:uPerm,block:'content.edit'},this.parallel());
        module.router.addRoute('GET /screens/edit/alias/:alias.:html', editContentForm,{admin:true,permit:uPerm,block:'content.edit'},this.parallel());
        module.router.addRoute('GET /screens/new',createContentForm,{admin:true,permit:vPerm,block:'content.create'},this.parallel());
        // module.router.addRoute('GET /content/delete/:id',deleteContent,{admin:true,permit:dPerm},this.parallel());
        module.router.addRoute('POST /screens/update/id/:id',updateContent,{admin:true,permit:uPerm},this.parallel());
        module.router.addRoute('POST /screens/update/alias/:alias.html',updateContent,{admin:true,permit:uPerm},this.parallel());

	calipso.silly("Screenwiki: Routes initialized.");
     },
     function done(err) {
          var themeConfig = calipso.availableThemes['screenswiki']; // Reference to theme.json
           if (themeConfig) {
             calipso.themes.Theme(themeConfig, function(err, theme){
                 screenstheme=theme;
             });
           } else {
             calipso.silly("screenswiki theme was not configured.");
           }
           if (screenstheme == null) {
              err="Screenstheme was not loaded." + err;
           }
           next(err);
     });
};
...
{
    res.layout='view';
    res.theme=screenstheme;

    // switch the theme.
    // console.dir(screenstheme);
    screenstheme.renderItem(req,res,template,block,{"screenstest_renderItem":"Test content  for block screens.test from Screenswiki module."},next);
...
function showScreenContentList(req, res, template, block, next)
{
    ...
    calipso.lib.step( 
         ...
         function done(err, output) {
               res.theme.renderItem(req,res, template, block, {contents: output.contents, pager: output.pager}, next);
         }
     );
}
...
function showScreenContentFromAlias(req, res, template, block,  next) {
    ...
    getContent(req, res, {alias: alias}, function(err, content) { 
       ...
       res.theme.renderItem(req,res, template, 'screens.item.body', {body: content.content}, next);
    });
}
...
function showScreenContentExpanded(req, res, template, block, next) {
    ...
    calipso.lib.step(
       ...
       function prepareDom(err, content) {
           if (err) throw("Error received in prepareDom, and received=" + err); 
       ...
       function populatePartials(err, dom, content) {
          if (err) throw("Error received in populatePartials, and received=" + err); 
          // calipso.silly("Step 3: content:" + content.content);
          // NOT getting printed:

          var partials=getPartials(dom);

          if(partials==null) partials=[];
          ...
          if (!forked) this.parallel()(undefined, partials);
       },
       function replacePartialsStep(err, content, dom, partials) {
           if (err) throw("Error received in replacePartials, and received=" + err); 
       ...
       function done(err, content) {
          if (err) console.log("Error received in done(), and received=" + err); 
          ...
          if (err) {
               ...
               if(content) {
                ...
                else body="Some error while processing this content. (Content object was null.)";
               if (body == null) body="Some error while processing this content.";
          }
          res.theme.renderItem(req,res, template, 'screens.item.body', {body: body, alias: alias}, next);
...
function getContent(req, res, inputs, next)
{
  ...
  var contentFn=function (err, content) {
   ...
   if (inputs.id != null) Content.findById(inputs.id, contentFn);
   else Content.findOne({alias: inputs.alias},  contentFn);
}
...
function fetchPartialsInThisSource(source, partials, next)
{
    ...
    getContent(null, null, {alias: source}, function(err, text) {
         if (err || !text) {
             ...
             for(var i=0; i<partials.length; i++) {
                var partial=partials[i];;
                if ( partial.source != source ) continue;
...
function showContent(req,res,template,block,next,err,content,format) {
  ...
  if(format === "html") {
     ...
     if (res.theme != null) {
           res.theme.renderItem(req,res, template, block, {screencontent32:content.content}, next);
     } else {
...
function createContentForm(req,res,template,block,next) {
    ...
    calipso.form.render(form,values,req,function(form) {
      res.theme.renderItem(req,res,form,'screens.item.body',{},next);
    });
}
